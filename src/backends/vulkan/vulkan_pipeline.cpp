    ...
    const DynamicRenderingFormats* dynamic_formats)
{
    ...
    multi_sample_state.alphaToCoverageEnable = pipeline_state.multiple_sample_state.alpha_to_coverage_enable ? VK_TRUE : VK_FALSE;
    multi_sample_state.alphaToOneEnable = pipeline_state.multiple_sample_state.alpha_to_one_enable ? VK_TRUE : VK_FALSE;
    multi_sample_state.sampleShadingEnable = pipeline_state.multiple_sample_state.sample_shading_enable ? VK_TRUE : VK_FALSE;
    multi_sample_state.minSampleShading = 1.0f;
    multi_sample_state.pSampleMask = nullptr;
    multi_sample_state.rasterizationSamples = get_vulkan_sample_count_flag_bit(pipeline_state.multiple_sample_state.sample_count);
    ...
    for (uint32_t i = 0; i < color_attachment_count; ++i) {
        ...
        blend_attachment_state.blendEnable = pipeline_state.blend_state.blend_enable ? VK_TRUE : VK_FALSE;
        blend_attachment_state.colorWriteMask = get_vulkan_color_component_flag_bits(pipeline_state.blend_state.color_mask);
        blend_attachment_state.alphaBlendOp = get_vulkan_blend_op(pipeline_state.blend_state.alphaBlendOp);
        blend_attachment_state.colorBlendOp = get_vulkan_blend_op(pipeline_state.blend_state.colorBlendOp);
        blend_attachment_state.srcAlphaBlendFactor = get_vulkan_blend_factor(pipeline_state.blend_state.srcalphablend_factor);
        blend_attachment_state.dstAlphaBlendFactor = get_vulkan_blend_factor(pipeline_state.blend_state.dstalphablend_factor);
        blend_attachment_state.srcColorBlendFactor = get_vulkan_blend_factor(pipeline_state.blend_state.srccolorblend_factor);
        blend_attachment_state.dstColorBlendFactor = get_vulkan_blend_factor(pipeline_state.blend_state.dstcolorblend_factor);
    }
    ...
    rasterization_state.cullMode = get_vulkan_cull_mode(pipeline_state.raster_state.cull_mode);
    rasterization_state.frontFace = get_vulkan_front_face(pipeline_state.raster_state.front_face);
    rasterization_state.depthBiasEnable = pipeline_state.raster_state.depth_bias ? VK_TRUE : VK_FALSE;
    rasterization_state.depthClampEnable = pipeline_state.raster_state.depth_clamp ? VK_TRUE : VK_FALSE;
    ...
    input_assembly_state.topology = get_vulkan_topology(pipeline_state.primitive_type);
